...
onready var active_item_label = $ItemText
onready var slots = hotbar.get_children()

func _ready():
	var _err1 =  PlayerInventory.connect("active_item_updated", self, "update_active_item_label")
	for i in range(slots.size()):
		var _err2 = PlayerInventory.connect("active_item_updated", slots[i], "refresh_style")
